    ...
    <div class="col-lg-10">
        <form action="/produk/{{ $produk->id }}" method="post">
                    ...
                    <div class="col-sm-10">
                        <select class="form-select @error('toko_id') is-invalid @enderror" id="toko_id" name="toko_id">
                            ...
                            @foreach ($tokos as $toko)
                                @if (old('toko_id', $produk->toko_id) == $toko->id)
            ...
            <div class="mb-3 row">
                ...
                <div class="col-sm-10">
                    <select class="form-select @error('kategori') is-invalid @enderror" id="kategori_id" name="kategori_id">
                        <option value="">-- Pilih Kategori --</option>
                        @foreach ($produk_kategoris as $produk_kategori)
                            @if (old('toko_id', $produk->kategori_id) == $produk_kategori->id)
                                <option value="{{ $produk_kategori->id }}" selected>
                                    {{ $produk_kategori->nama_kategori }}
                                </option>
                            @else
                                <option value="{{ $produk_kategori->id }}">{{ $produk_kategori->nama_kategori }}
                                </option>
                            @endif
                        @endforeach
                    </select>
            ...
            <div class="mb-3 row">
                ...
                <div class="col-sm-10">
                    <input type="number" class="form-control @error('harga_beli') is-invalid @enderror" id="harga_beli"
                        name="harga_beli" value="{{ old('harga_beli', $produk->harga_beli) }}">
                    @error('harga_beli')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
